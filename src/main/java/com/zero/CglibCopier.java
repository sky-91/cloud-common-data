 ...
 * @Description: CglibCopier
 * @Jdk: 8
 */
public class CglibCopier {

    private static final Logger logger = LoggerFactory.getLogger(CglibCopier.class);

    /**
     * 使用并发引用hashmap存储BeanCopier实例
     */
    ...
    public static <S,T> BeanCopier getBeanCopierWithNoConverter(Class<S> sourceClass, Class<T> targetClass) {
        ...
        if (!bencopierMap.containsKey(beanKeyStr)) {
            synchronized (CglibCopier.class) {
                if (!bencopierMap.containsKey(beanKeyStr)) {
    ...
    public static <T> T copy(Object source, Class<T> clazz) {
        ...
        try {
            ...
            logger.warn("CglibCopier copy exception:{}", e.getMessage());
    ...
    public static <T> T copyWithConverter(Object source, Class<T> clazz, Converter converter) {
        ...
        try {
            ...
            logger.warn("CglibCopier copy with converter exception:{}", e.getMessage());
            throw new RuntimeException(e);
        }
        return target;
    }
